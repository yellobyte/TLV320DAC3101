/*
  ...
  frequencies below the set corner frequency get strongly attenuated. Q is chosen differently 
  to keep -3dB attenuation at fc. The audio signal is output on both the speaker and 
  headphone sockets.
...
// defines the -3dB corner frequency of the high pass filters
#define FREQU_C       2000          // Hz
...
tlv320_filter_param_t filterA, filterB;  // keeps the filter parameters
...
void setup() {
  ...
  Serial.println("\nrunning example \"BiQuad High Pass Filter (4th order)\":");
  ...
  filterA.fc = (float)FREQU_C;               // -3dB corner frequency
  filterA.Q = 1 / 0.7654;

  // calculate coefficients for first filter block
  ...
  // setting filter parameters for second filter block
  filterB.fc = (float)FREQU_C;                   // -3dB corner frequency
  filterB.Q = 1 / 1.8478;

  // calculate coefficients for second filter block
  if (!dac.calcDACFilterCoefficients(SAMPLERATE_HZ, TLV320_FILTER_TYPE_HIGH_PASS,
